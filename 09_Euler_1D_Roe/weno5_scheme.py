...
def weno5L(u):  # uL at i+1/2 很多时候要用u[i-1]
    uL = np.zeros_like(u)
    for i in range(2,len(u[0,:])-3):  # uL从index=2开始更新，index=-4结束（包含2和-4）
        for k in range(3):
            uL[k,i+1] = weightsL(u[k,i - 2], u[k,i - 1], u[k,i], u[k,i + 1], u[k,i + 2])  # 转化为index从3到-3的i-/2，与以往的代码稍微有点区别
    return uL
...
def weno5R(u):  # uR at i-1/2
    uR = np.zeros_like(u)
    for i in range(3, len(u[0,:]) - 2):  # # uR从index=3开始更新，index=-3结束（包含3和-3）
        for k in range(3):
            uR[k,i] = weightsR(u[k,i - 2], u[k,i - 1], u[k,i], u[k,i + 1], u[k,i + 2])
    return uR
